...
import {
  createFromRequestAction,
  rejectRequestAction,
  markRequestPaidAction,
  markRequestUnpaidAction,
} from './actions';
...
type ActionFn = typeof createFromRequestAction;
...
export default function RequestsManager({
  ...
  function run(id: string, fn: ActionFn) {
  ...
  const toVerify = requests.filter(r => r.status === 'pending');
  const paid = requests.filter(r => r.status === 'paid');

  const card = (r: SignupRequestView) => (
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {r.status === 'paid' && (
              <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-extrabold text-emerald-700">
                <Icon name="check" size={10} strokeWidth={3} /> BAYAD
              </span>
            )}
          </div>
          ...
          <div className="mt-2 grid gap-1 text-[12.5px]">
            ...
            <div>
              <span className="font-bold text-ink-2/60">Dapat bayad:</span>{' '}
              <span className="font-extrabold text-ink">₱{expectedAmount.toLocaleString('en-PH')}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="font-bold text-ink-2/60">GCash ref:</span>
              {r.paymentReference ? (
                <button
                  type="button"
                  onClick={() => copyRef(r.paymentReference)}
                  className="inline-flex items-center gap-1 rounded bg-ink/5 px-1.5 py-0.5 font-mono text-[12px] hover:bg-ink/10"
                  title="I-copy"
                >
                  {r.paymentReference}
                  {copied === r.paymentReference
                    ? <Icon name="check" size={11} className="text-emerald-600" />
                    : <span className="text-[10px] font-sans font-bold text-ink-2/50">copy</span>}
                </button>
              ) : (
                <span className="text-ink-2/50">— (walang inilagay)</span>
              )}
            </div>
        ...
        <div className="flex flex-row sm:flex-col gap-2 shrink-0">
          {r.status === 'pending' ? (
            <button
              onClick={() => run(r.id, markRequestPaidAction)}
              disabled={pending}
              className="btn-primary text-[12.5px] px-3 py-2 whitespace-nowrap"
              title="I-click kapag na-verify mo na ang bayad sa resibo"
            >
              {busy === r.id ? 'Sandali…' : <>Bayad na ✓</>}
            </button>
          ) : (
            <>
              <button
              ...
              <button
                onClick={() => run(r.id, markRequestUnpaidAction)}
                disabled={pending}
                className="btn-ghost text-[12px] px-3 py-2 text-ink-2/70 whitespace-nowrap"
                title="Balik sa pending"
              >
                Undo paid
              </button>
            </>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Paid — ready to approve */}
      {paid.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-emerald-700">
            Bayad na — ready to approve ({paid.length})
          </h2>
          {paid.map(card)}
        </section>
      )}
      ...
      <section className="space-y-3">
        <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-ink-2/60">
          Bagong request — i-verify ang bayad ({toVerify.length})
        </h2>
        {toVerify.length > 0
          ? toVerify.map(card)
          : (
            <GlassCard className="p-8 text-center text-[13.5px] font-semibold text-ink-2/60">
              Walang bagong request. 🎉 Lalabas dito ang mga bagong signup.
            </GlassCard>
          )}
      </section>
